  ...
  return (
    <div className="flex flex-col gap-1.5">
      <label htmlFor={id} className="text-sm font-semibold text-slate-700 flex items-center gap-1">
        {label}<span className="text-amber-500 text-xs">*</span>
      </label>
      <div className="flex items-center overflow-hidden border-2 rounded-xl transition-all duration-200 bg-white border-slate-200 focus-within:border-amber-500 focus-within:ring-2 focus-within:ring-amber-500/10 shadow-sm">
    ...
    <AuthLayout>
      <div className="flex flex-col items-center text-center animate-fade-up">
        ...
        <div className="relative mb-6">
          ...
          />
          <div className="absolute inset-0 rounded-full animate-pulse opacity-35"
            style={{ background: 'radial-gradient(circle, #E98A15, transparent)', transform: 'scale(1.4)' }}
          />
          {/* Icon container */}
          <div
            className="relative w-20 h-20 rounded-full flex items-center justify-center bg-amber-50 border border-amber-200"
            style={{ boxShadow: '0 4px 20px rgba(233,138,21,0.15)' }}
          >
            <svg className="w-9 h-9 text-amber-600" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round">
        ...
        <p className="text-sm text-slate-600 mb-1">We've sent a verification link to</p>
        <p className="text-sm font-bold text-amber-600 mb-6 break-all">{email}</p>

        {/* Polling status pill */}
        <div
          className="flex items-center gap-2.5 px-4 py-3 rounded-xl mb-7 w-full border border-amber-200 bg-amber-50"
        >
          {checking ? (
            <Spinner />
          ) : (
          ...
          <span className="text-sm text-amber-800 text-left font-medium">
        ...
        <div className="w-full flex flex-col items-center gap-2">
          {resendSuccess && (
            <p className="text-xs text-emerald-600 font-medium animate-slide-up">
              ✓ Verification email resent successfully
            </p>
          )}
          {resendError && (
            <p className="text-xs text-red-600 animate-slide-up">{resendError}</p>
          )}
          <button
            type="button"
            disabled={resendCooldown > 0}
            onClick={handleResend}
            className="text-sm font-semibold text-amber-600 hover:text-amber-700 transition-colors disabled:opacity-40 disabled:cursor-not-allowed"
          >
            {resendCooldown > 0
          ...
          <button
            type="button"
            onClick={onBack}
            className="mt-1 text-xs text-slate-500 hover:text-slate-700 transition-colors flex items-center gap-1"
          >
  ...
  return (
    <AuthLayout>
      ...
      <div
        ...
        style={{ background: 'rgba(233,138,21,0.1)', color: '#D97706', border: '1px solid rgba(233,138,21,0.2)' }}
      >
        <svg className="w-3 h-3" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round">
      ...
      <h1 className="font-display text-2xl font-bold text-slate-900">{t('auth.signUpTitle')}</h1>
      <p className="mt-1.5 text-sm text-slate-600 leading-relaxed">{t('auth.signUpSubtitle')}</p>

      <form className="mt-7 space-y-4" onSubmit={handleRegister}>

        {/* Full Name */}
        <div className="flex flex-col gap-1.5">
          <label htmlFor="signup-name" className="text-sm font-semibold text-slate-700 flex items-center gap-1">
            {t('auth.nameLabel')}<span className="text-amber-500 text-xs">*</span>
          </label>
          <div className="flex items-center overflow-hidden border-2 rounded-xl transition-all duration-200 bg-white border-slate-200 focus-within:border-amber-500 focus-within:ring-2 focus-within:ring-amber-500/10 shadow-sm">
            <div className="flex items-center px-3.5 flex-shrink-0 text-slate-400">
              <svg className="w-4 h-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
        ...
        {/* Email */}
        <div className="flex flex-col gap-1.5">
          <label htmlFor="signup-email" className="text-sm font-semibold text-slate-700 flex items-center gap-1">
            {t('auth.emailLabel')}<span className="text-amber-500 text-xs">*</span>
          </label>
          <div className="flex items-center overflow-hidden border-2 rounded-xl transition-all duration-200 bg-white border-slate-200 focus-within:border-amber-500 focus-within:ring-2 focus-within:ring-amber-500/10 shadow-sm">
            <div className="flex items-center px-3.5 flex-shrink-0 text-slate-400">
              <svg className="w-4 h-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
        ...
        {confirmPassword.length > 0 && (
          <div className={`flex items-center gap-2 text-xs font-medium ${password === confirmPassword ? 'text-emerald-600' : 'text-red-600'}`}>
      ...
      <div className="mt-7 pt-6 border-t border-slate-200">
        <p className="text-center text-sm text-slate-500">
          {t('auth.haveAccount')}{' '}
          <Link to="/signin" className="font-semibold text-amber-600 hover:text-amber-500 transition-colors underline-offset-2 hover:underline">
