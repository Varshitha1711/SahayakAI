  ...
  return (
    <div className="flex flex-col gap-1.5">
      <label htmlFor={id} className="text-sm font-semibold text-indigo-200/90 flex items-center gap-1">
        {label}<span className="text-marigold-500 text-xs">*</span>
      </label>
      <div
...
export default function SignUp() {
  ...
  const handleRegister = async (e) => {
    ...
    try {
      ...
      const msg = err.response?.data?.detail || 'An error occurred during registration. Please try again.';
      setError(msg);
    } finally {
      ...
      <form className="mt-7 space-y-4" onSubmit={handleRegister}>
        ...
        <div className="flex flex-col gap-1.5">
          <label htmlFor="signup-name" className="text-sm font-semibold text-indigo-200/90 flex items-center gap-1">
            {t('auth.nameLabel')}<span className="text-marigold-500 text-xs">*</span>
          </label>
          <div
            className="flex items-center overflow-hidden border-2 rounded-xl transition-all duration-200 bg-white/5 border-white/10 focus-within:border-amber-500 focus-within:bg-white/[0.07]"
          >
        ...
        <div className="flex flex-col gap-1.5">
          <label htmlFor="signup-email" className="text-sm font-semibold text-indigo-200/90 flex items-center gap-1">
            {t('auth.emailLabel')}<span className="text-marigold-500 text-xs">*</span>
          </label>
          <div
            className="flex items-center overflow-hidden border-2 rounded-xl transition-all duration-200 bg-white/5 border-white/10 focus-within:border-amber-500 focus-within:bg-white/[0.07]"
          >
